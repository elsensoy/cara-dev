...
from sound.cara_audio_io import recognize_from_microphone, speak_text, get_recognition_stats

# Hardware & Emotion Imports 
#from sentiment_detection import detect_emotion, react_to_emotion
#from serial_control import initialize_arduino
# HeadController is in a file named head_controller.py
...
# NEW FUNCTION ADDED HERE
    def send_correction_signal(self, true_emotion):
        """Tells the vision node to save recent frames with this new label"""
        msg = String()
        # Format: "SAVE:<label>"
        msg.data = f"SAVE:{true_emotion}"
        self.pub_train.publish(msg)
        self.get_logger().info(f"Sent training correction: {true_emotion}")
...
# 5. LOAD BASE PROMPT
with open("cara_memory/cara_prompt.json", "r") as f:
...
If the user corrects your perception of their emotion (e.g., "I'm not sad, I'm thinking"), 
you MUST acknowledge it and include this exact tag at the end of your response:
[LEARN: correct_emotion]
Example: "Oh, I see! You are thinking. [LEARN: thinking]"
"""
# 7. INITIALIZE MODEL
full_system_instruction = base_system_instruction + learning_instruction + get_memory_context()
model = genai.GenerativeModel(
    'models/gemini-3-flash-preview',             # <--- REPLACE WITH THIS
...
def main():
    ...
    while True:
        ...
        # === NEW BLOCK: CHECK FOR LEARNING TAGS ===
        if "[LEARN:" in cara_reply:
            try:
                # Extract the emotion inside the brackets
                start = cara_reply.find("[LEARN:") + 7
                end = cara_reply.find("]", start)
                true_emotion = cara_reply[start:end].strip().lower()
                
                # Send the signal to the Vision Node
                print(f" >> LEARNING TRIGGERED: Retraining on '{true_emotion}'")
                ros_node.send_correction_signal(true_emotion)
                
                # Optional: Clean the tag out so it isn't spoken aloud
                cara_reply = cara_reply.replace(f"[LEARN: {true_emotion}]", "")
                cara_reply = cara_reply.replace(f"[LEARN:{true_emotion}]", "")
            except Exception as e:
                print(f"Error parsing learning tag: {e}")
        # ==========================================
...
# --- F. ACTIONS ---
        ...
        # Hardware Reaction
        # TODO: Re-integrate Arduino/Serial logic here when the module is ready.
        # (Logic removed because 'serial_control' module is currently missing)
        print(f"[Hardware]: Skipping Arduino command for emotion '{text_emotion}'")

        # NOTE: Ensure your ROS head controller isn't fighting this one!
        # This part is fine if your HeadController class is working
        head_controller.express_emotion(text_emotion) 
        
        # react_to_emotion(text_emotion) # <--- Comment this out too if it relies on Arduino
